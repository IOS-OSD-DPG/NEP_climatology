...
def ios_time_scatter(time_data, profile_data, out_dir, instrument, var='oxygen'):
    # USED THIS FUNCTION
...
def ios_get_proftime(ncfile):
    ...
    return years, prof_year_counts, time_subset


def ios_time_scatter_v2(nclist, out_dir, instrument, var='oxygen'):
    # USED THIS FUNCTION
    # Most recent temporal plotting function
    # nclist: list of netcdf file paths

    # Commented out unfinished scatter plot for all seasons
    ...
    if len(nclist) > 1:
        ...
        for i in range(len(nclist)):
            years, prof_year_counts, time_subset = ios_get_proftime(nclist[i])
            time_subset_list.append(time_subset)  # To use later...
    else:
        multifile = False

    # # Call function here
    # years, prof_year_counts, time_subset = ios_get_proftime(nclist[0])
    #
    # # Start building a list of the time subsets for all files
    # time_subset_list = [time_subset]
    #
    # # Make a scatter plot covering all 30 years
    # fig, ax = plt.subplots()
    # ax.scatter(years, prof_year_counts, color='b')
    #
    # # Add rest of data if multifile is True
    # if multifile:
    #     for i in range(1, len(nclist)):
    #         years, prof_year_counts, time_subset = ios_get_proftime(nclist[i])
    #         time_subset_list.append(time_subset)  # To use later...
    ...
    # Flatten the time list into 1d array
    time_subset_array = to_datetime(list(chain(*time_subset_list)))
    ...
    for y in range(len(years)):
        for s in range(nszn):
            ...
            szn_end = 3 * s + 3  # get number of end month of season
            subsetter = np.where(
                (time_subset_array.year == years[y]
                 ) & (time_subset_array.month >= szn_start
                      ) & (time_subset_array.month <= szn_end))
            prof_szn_counts[y, s] = len(time_subset_array[subsetter])
    ...
    return [png_name_szn]  # [png_name, png_name_szn]
...
def ios_runBOT():
    ...
    # ios_time_scatter(data.time.data, dest_dir, 'BOT', indices)
...
def ios_runPCTD():
    # CTD_dir = '/home/hourstonh/Documents/climatology/data/oxy_clim/IOS_CIOOS/IOS_CTD_Profiles'
    CTD_dir = 'C:\\Users\\HourstonH\\Documents\\NEP_climatology\\data\\IOS_CIOOS\\'
    CTDfiles = glob.glob(CTD_dir + 'IOS_CTD_Profiles_Oxy*.nc', recursive=False)
    CTDfiles.sort()

    # Output directory
    # dest_dir = '/home/hourstonh/Documents/climatology/data_explore/oxygen/IOS/PCTD/'
    dest_dir = 'C:\\Users\\HourstonH\\Documents\\NEP_climatology\\data_explore\\oxygen\\'
    ...
    # Time scatter plots vs year
    png_names += ios_time_scatter_v2(CTDfiles, dest_dir, 'CTD')

    # Iterate through the seasons
    # for s in szns:
    #     outname = ios_map_dist_v2(CTDfiles, dest_dir, 'CTD', left_lon=-160,
