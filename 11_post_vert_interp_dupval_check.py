...
interp_dir = '/home/hourstonh/Documents/climatology/data/value_vs_depth/' \
             '10_vertical_interp/'

infiles = glob.glob(interp_dir + '*Temp*value_vs_depth*.csv')
print(len(infiles))
...
for j in range(len(infiles)):
    ...
    out_dir = '/home/hourstonh/Documents/climatology/data/value_vs_depth/' \
              '11_replicate_check/'
    
    outname = basename(infiles[j]).replace('.csv', '_rep_val_check.csv')
    
    df_in.to_csv(out_dir + outname, index=False)
    
    print(len(df_in))
    print(len(df_in.loc[df_in.Replicate_val_flag == 1]))
    print(len(df_in.loc[df_in.Replicate_val_flag == 1]) / len(df_in))
